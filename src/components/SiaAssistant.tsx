...
export default function SiaAssistant({
  ...
  const [reelProfileImageEnabled, setReelProfileImageEnabled] = useState(true);
  ...
  const [reelOverlayPosition, setReelOverlayPosition] = useState<'top' | 'center' | 'bottom'>('top');
  ...
  const [reelBandColor, setReelBandColor] = useState('#ffffff');
  const [reelTextColor, setReelTextColor] = useState('#111318');
  const [reelCtaBackground, setReelCtaBackground] = useState('#111318');
  const [reelCtaColor, setReelCtaColor] = useState('#ffffff');
  const [reelFrameBackground, setReelFrameBackground] = useState('#ffffff');
  const [reelFrameTextColor, setReelFrameTextColor] = useState('#111318');
                  ...
                  <div className="p-3 sm:p-4">
                    ...
                    {creativeMode === 'frame' ? (
                      <div className="space-y-4">
                        ...
                        {reelCustomizerOpen && (
                          <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="reel-customizer-title">
                            <div className="max-h-[min(760px,calc(100vh-32px))] w-full max-w-2xl overflow-y-auto rounded-2xl border border-amber-200 bg-white p-5 shadow-2xl">
                              ...
                              <div className="mt-5 grid gap-5 lg:grid-cols-[220px_1fr] lg:items-start">
                                <div><p className="mb-2 text-[10px] font-black uppercase tracking-[0.1em] text-gray-500">Prévia em tempo real</p><div className="relative mx-auto flex aspect-[9/16] w-full max-w-[220px] flex-col overflow-hidden rounded-2xl border border-gray-200 shadow-lg" style={{ backgroundColor: reelFrameBackground, color: reelFrameTextColor }}><div className="flex min-h-[16%] flex-none flex-col justify-center gap-1 px-3 py-2"><div className="flex items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center overflow-hidden rounded-full bg-amber-200 text-[10px] font-black text-gray-900">{reelProfileImageEnabled && reelProfileImage ? <img src={reelProfileImage} alt="" className="h-full w-full object-cover" /> : reelProfileImageEnabled ? (reelProfileName || currentStore.name).slice(0, 1).toUpperCase() : null}</span><span className="min-w-0"><strong className="block truncate text-[9px]" style={{ color: reelFrameTextColor }}>{reelProfileName || currentStore.name}</strong><small className="block truncate text-[8px] opacity-60">{reelProfileHandle || currentStore.profileHandle || `@${normalizeSearch(currentStore.name).replace(/\s+/g, '')}`}</small></span></div>{reelOverlayBand && reelOverlayPosition === 'top' && <div className="w-full break-words whitespace-normal px-1 text-center text-[8px] font-bold leading-tight" style={{ color: reelFrameTextColor }}>{reelOverlayText || 'Texto da legenda'}</div>}</div><div className="relative min-h-0 flex-1 overflow-hidden bg-[var(--reel-frame-bg)]" style={{ '--reel-frame-bg': reelFrameBackground } as React.CSSProperties}>{reelVideoPreview && <video src={reelVideoPreview} muted playsInline autoPlay loop aria-hidden="true" className="absolute inset-0 h-full w-full object-cover opacity-30 blur-xl" />}<video src={reelVideoPreview || undefined} muted playsInline autoPlay loop className="relative h-full w-full" style={{ objectFit: reelOverlayZoom < 1 ? 'contain' : 'cover', transform: `translate(${reelOverlayZoom < 1 ? 0 : -reelPanX * 12}%, ${reelOverlayZoom < 1 ? 0 : -reelPanY * 12}%) scale(${reelOverlayZoom})`, transformOrigin: 'center center' }} />{reelOverlayBand && reelOverlayPosition !== 'top' && <div className={`absolute left-3 right-3 rounded-lg px-2 py-2 text-center text-[10px] font-black leading-tight ${reelOverlayPosition === 'bottom' ? 'bottom-3' : 'top-1/2 -translate-y-1/2'}`} style={{ backgroundColor: reelBandColor, color: reelTextColor }}>{reelOverlayText || 'Texto da legenda'}</div>}</div><div className="flex min-h-[16%] flex-none items-center justify-center px-3 py-2">{reelOverlayCta && <span className="w-full rounded-lg px-2 py-2 text-center text-[9px] font-black" style={{ backgroundColor: reelCtaBackground, color: reelCtaColor }}>{reelOverlayCta}</span>}</div></div><p className="mt-2 text-center text-[10px] leading-relaxed text-gray-500">A prévia acompanha suas alterações antes da geração.</p></div>
                              <div className="mt-5 grid gap-4">
                                ...
                                <div className="grid gap-4 sm:grid-cols-2"><div><label className="text-[10px] font-black uppercase tracking-[0.1em] text-gray-500" htmlFor="reel-custom-position">Posição da faixa</label><select id="reel-custom-position" value={reelOverlayPosition} onChange={event => { setReelOverlayPosition(event.target.value as 'top' | 'center' | 'bottom'); clearGeneratedReels(); }} className="mt-2 h-10 w-full rounded-lg border border-gray-200 bg-white px-3 text-xs font-semibold text-gray-800"><option value="top">Superior</option><option value="center">Centro</option><option value="bottom">Inferior</option></select></div><div><label className="text-[10px] font-black uppercase tracking-[0.1em] text-gray-500" htmlFor="reel-custom-zoom">Enquadramento: {Math.round(reelOverlayZoom * 100)}%</label><input id="reel-custom-zoom" type="range" min="0.75" max="2" step="0.01" value={reelOverlayZoom} onChange={event => { setReelOverlayZoom(Number(event.target.value)); clearGeneratedReels(); }} className="mt-5 w-full accent-amber-500" /><span className="mt-1 block text-[10px] text-gray-400">Abaixo de 100% afasta o vídeo</span></div></div>
                                <div className="grid gap-3 sm:grid-cols-2"><div><label className="text-[10px] font-black uppercase tracking-[0.1em] text-gray-500" htmlFor="reel-pan-x">Enquadramento horizontal</label><input id="reel-pan-x" type="range" min="-1" max="1" step="0.01" value={reelPanX} onChange={event => { setReelPanX(Number(event.target.value)); clearGeneratedReels(); }} className="mt-3 w-full accent-amber-500" /></div><div><label className="text-[10px] font-black uppercase tracking-[0.1em] text-gray-500" htmlFor="reel-pan-y">Enquadramento vertical</label><input id="reel-pan-y" type="range" min="-1" max="1" step="0.01" value={reelPanY} onChange={event => { setReelPanY(Number(event.target.value)); clearGeneratedReels(); }} className="mt-3 w-full accent-amber-500" /></div></div>
                                <div className="grid gap-3 sm:grid-cols-2"><label className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-600">Cor da faixa<input type="color" value={reelBandColor} onChange={event => { setReelBandColor(event.target.value); clearGeneratedReels(); }} className="h-7 w-10 cursor-pointer rounded border-0 bg-transparent p-0" /></label><label className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-600">Cor do texto<input type="color" value={reelTextColor} onChange={event => { setReelTextColor(event.target.value); clearGeneratedReels(); }} className="h-7 w-10 cursor-pointer rounded border-0 bg-transparent p-0" /></label><label className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-600">Fundo do CTA<input type="color" value={reelCtaBackground} onChange={event => { setReelCtaBackground(event.target.value); clearGeneratedReels(); }} className="h-7 w-10 cursor-pointer rounded border-0 bg-transparent p-0" /></label><label className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-600">Texto do CTA<input type="color" value={reelCtaColor} onChange={event => { setReelCtaColor(event.target.value); clearGeneratedReels(); }} className="h-7 w-10 cursor-pointer rounded border-0 bg-transparent p-0" /></label><label className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-600">Fundo do vídeo/painel<input type="color" value={reelFrameBackground} onChange={event => { setReelFrameBackground(event.target.value); clearGeneratedReels(); }} className="h-7 w-10 cursor-pointer rounded border-0 bg-transparent p-0" /></label><label className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-600">Texto do painel<input type="color" value={reelFrameTextColor} onChange={event => { setReelFrameTextColor(event.target.value); clearGeneratedReels(); }} className="h-7 w-10 cursor-pointer rounded border-0 bg-transparent p-0" /></label></div>
                        ...
                        <label className="mt-2 flex items-center gap-2 text-[10px] font-bold text-gray-600"><input type="checkbox" checked={reelProfileImageEnabled} onChange={event => { setReelProfileImageEnabled(event.target.checked); clearGeneratedReels(); }} className="h-4 w-4 accent-amber-500" />Mostrar foto de perfil</label>
