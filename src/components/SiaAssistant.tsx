                          ...
                          <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="reel-customizer-title">
                            <div className="max-h-[min(760px,calc(100vh-32px))] w-full max-w-2xl overflow-y-auto rounded-2xl border border-amber-200 bg-white p-5 shadow-2xl">
                              ...
                              <div className="mt-5 grid gap-5 lg:grid-cols-[220px_1fr] lg:items-start">
                                <div><p className="mb-2 text-[10px] font-black uppercase tracking-[0.1em] text-gray-500">Prévia em tempo real</p><div className="relative mx-auto aspect-[9/16] w-full max-w-[220px] overflow-hidden rounded-2xl border border-gray-200 shadow-lg" style={{ backgroundColor: reelFrameBackground, color: reelFrameTextColor }}><div className="flex h-[23%] items-center gap-2 px-3"><span className="grid h-8 w-8 shrink-0 place-items-center overflow-hidden rounded-full bg-amber-200 text-[10px] font-black text-gray-900">{reelProfileImageEnabled && reelProfileImage ? <img src={reelProfileImage} alt="" className="h-full w-full object-cover" /> : reelProfileImageEnabled ? (reelProfileName || currentStore.name).slice(0, 1).toUpperCase() : null}</span><span className="min-w-0"><strong className="block truncate text-[9px]" style={{ color: reelFrameTextColor }}>{reelProfileName || currentStore.name}</strong><small className="block truncate text-[8px] opacity-60">{reelProfileHandle || currentStore.profileHandle || `@${normalizeSearch(currentStore.name).replace(/\s+/g, '')}`}</small></span></div>{reelOverlayBand && reelOverlayPosition === 'top' && <div className="absolute left-[39%] right-3 top-[8%] z-10 line-clamp-2 px-1 text-center text-[8px] font-bold leading-tight" style={{ color: reelFrameTextColor }}>{reelOverlayText || 'Texto da legenda'}</div>}<div className="relative h-[61%] overflow-hidden bg-black"><video src={reelVideoPreview || undefined} muted playsInline autoPlay loop className="h-full w-full object-cover" style={{ transform: `translate(${-reelPanX * 12}%, ${-reelPanY * 12}%) scale(${reelOverlayZoom})` }} />{reelOverlayBand && reelOverlayPosition !== 'top' && <div className={`absolute left-3 right-3 rounded-lg px-2 py-2 text-center text-[10px] font-black leading-tight ${reelOverlayPosition === 'bottom' ? 'bottom-3' : 'top-1/2 -translate-y-1/2'}`} style={{ backgroundColor: reelBandColor, color: reelTextColor }}>{reelOverlayText || 'Texto da legenda'}</div>}</div><div className="flex h-[16%] items-center justify-center px-3">{reelOverlayCta && <span className="w-full rounded-lg px-2 py-2 text-center text-[9px] font-black" style={{ backgroundColor: reelCtaBackground, color: reelCtaColor }}>{reelOverlayCta}</span>}</div></div><p className="mt-2 text-center text-[10px] leading-relaxed text-gray-500">A prévia acompanha suas alterações antes da geração.</p></div>
