    ...
    <main className="relative z-10 grid min-h-screen lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <section className="flex items-center justify-center px-4 py-8 sm:px-8"><div className="w-full max-w-md rounded-[28px] border border-gray-200 bg-gray-50 p-5 shadow-2xl shadow-black/40 sm:p-7"><div className="mb-6"><p className="text-xs font-black uppercase tracking-[0.28em] text-brand-500">Login</p><h2 className="mt-1 text-2xl font-black text-gray-900">Acesse sua conta</h2></div>
        <form className="space-y-4" onSubmit={handleSubmit}>
          <Field label="E-mail" icon={<Mail size={18} className="text-slate-500" />}><input type="email" value={email} onChange={e => setEmail(e.target.value)} className={inputClass} placeholder="voce@empresa.com" autoComplete="email" /></Field>
          <Field label="Senha" icon={<Lock size={18} className="text-slate-500" />}><input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className={inputClass} placeholder="Minimo de 6 caracteres" autoComplete="current-password" /><button type="button" onClick={() => setShowPassword(v => !v)} className="text-slate-500" aria-label="Mostrar senha">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></Field>
          {<Field label="Codigo de Socio (opcional)" icon={<KeyRound size={18} className="text-amber-600" />}><input value={partnerCode} onChange={e => setPartnerCode(e.target.value.toUpperCase())} className={inputClass} placeholder="STORE4545" autoComplete="off" /></Field>}
