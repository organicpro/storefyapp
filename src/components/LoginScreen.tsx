...
export default function LoginScreen({ onLocalAccess }: LoginScreenProps) {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#050608] text-white">
      ...
      <main className="relative z-10 grid min-h-screen lg:grid-cols-[1.05fr_0.95fr]">
        <section className="flex flex-col justify-between px-6 py-7 sm:px-10 lg:px-14">
          ...
          <div className="max-w-2xl py-16">
            ...
            <h1 className="font-sans text-5xl font-black leading-[0.92] tracking-tight text-white sm:text-6xl xl:text-7xl">
        ...
        <section className="flex items-center justify-center px-4 py-8 sm:px-8">
          <div className="w-full max-w-md rounded-[30px] border border-gray-200 bg-gray-50 p-5 shadow-2xl shadow-black/40 backdrop-blur-2xl sm:p-7">
            <div className="mb-6 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.28em] text-brand-500">
                  Login
                </p>
                <h2 className="mt-1 font-sans text-2xl font-black text-gray-900">
                  Acesse sua conta
                </h2>
              </div>
              <div className="grid h-12 w-12 place-items-center rounded-xl border border-gray-200 bg-gray-100 text-brand-500">
                <Lock size={20} />
              </div>
            </div>

            {!isSupabaseConfigured && (
              <div className="mb-5 rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-900">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-gray-500">E-mail</span>
                <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-100 px-4 py-3 focus-within:border-brand-500/60">
                  <Mail size={18} className="text-slate-500" />
                  <input
                    type="email"
                    value={email}
                    onChange={event => setEmail(event.target.value)}
                    className="w-full bg-transparent text-sm font-semibold text-gray-900 outline-none placeholder:text-slate-600"
              ...
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-gray-500">Senha</span>
                <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-100 px-4 py-3 focus-within:border-brand-500/60">
                  ...
                  <button
                    ...
                    className="text-slate-500 transition hover:text-gray-900"
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-500 px-5 py-4 text-sm font-black text-black transition hover:bg-brand-200 disabled:cursor-not-allowed disabled:opacity-60"
              >
            ...
            <div className="mt-5 flex flex-col gap-3 text-center text-sm text-gray-500">
              {!isSupabaseConfigured && (
                <button
                  type="button"
                  onClick={onLocalAccess}
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 px-4 py-3 text-xs font-black uppercase tracking-[0.18em] text-gray-700 transition hover:bg-gray-100"
                >
                  <Store size={15} />
